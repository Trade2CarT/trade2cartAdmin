...
const AdminProcess = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 relative">
            <button onClick={() => navigate(-1)} className="text-sm text-brand-600 hover:underline mb-6">&larr; Back to Orders</button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <div className="bg-white p-6 rounded-xl shadow-lg" ref={searchContainerRef}>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setShowCustomModal(true)} className="flex items-center gap-1 bg-brand-50 text-brand-600 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-brand-100 transition">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 transition-shadow"
                            />
                            {isSearchFocused && (
                                <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg mt-1 max-h-60 overflow-y-auto shadow-2xl">
                                    {searchResults.length > 0 ? searchResults.map(item => (
                                        <div key={item.id} onClick={() => handleAddItem(item)} className="px-4 py-3 hover:bg-brand-50 cursor-pointer flex justify-between items-center transition-colors">
                                            <span>{item.name}</span>
                                            ...
                                            <tr key={item.billItemId} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 w-32">
                                                    <input
                                                        ...
                                                        className="w-full text-center border border-gray-300 rounded-md p-2 font-semibold text-gray-800 focus:ring-2 focus:ring-brand-500"
                                                    />
                                                </td>
                                                <td className="px-6 py-4 w-32">
                                                    <input
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <button onClick={handleSubmitBill} disabled={isSubmitting || billItems.length === 0} className="w-full py-4 bg-brand-600 text-white font-bold rounded-lg hover:bg-brand-700 transition-all text-lg disabled:bg-gray-400 disabled:cursor-not-allowed shadow-lg hover:shadow-xl">
                    ...
                    <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm relative">
                        ...
                        <div className="space-y-4 mb-8">
                            <div>
                                ...
                                <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)} placeholder="e.g. Copper Wire" className="w-full mt-1 p-3 border border-gray-300 rounded-lg font-semibold text-gray-900 focus:border-brand-500 focus:ring-2" />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Rate / Price (₹)</label>
                                <input type="number" value={customRate} onChange={(e) => setCustomRate(e.target.value)} placeholder="e.g. 50" className="w-full mt-1 p-3 border border-gray-300 rounded-lg font-semibold text-gray-900 focus:border-brand-500 focus:ring-2" />
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={handleAddCustom} className="flex-1 py-3 bg-brand-600 text-white font-bold rounded-lg shadow-md hover:bg-brand-700">
                                Add Item
                            </button>
                        </div>
                    </div>
