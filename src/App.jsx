    ...
    <div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <select value={sourceLocation} onChange={(e) => setSourceLocation(e.target.value)} className="w-full p-3 border-2 border-gray-200 rounded-xl focus:ring-0 focus:border-brand-500 font-bold text-gray-800">
            <option value="">-- Copy From --</option>
            {uniqueLocations.map(loc => <option key={loc} value={loc}>{loc}</option>)}
          </select>
          <input value={newLocation} onChange={(e) => setNewLocation(e.target.value)} placeholder="New Location Name (e.g. Mumbai)" className="w-full p-3 border-2 border-gray-200 rounded-xl focus:ring-0 focus:border-brand-500 font-bold text-gray-800" />
      ...
      <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100 mb-8 relative overflow-hidden">
        ...
        <form onSubmit={handleItemSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <div className="md:col-span-2 flex gap-4">
              <div className="flex-1">
                ...
                <input name="minRate" value={newItem.minRate} placeholder="Min Price" onChange={handleInputChange} className="w-full p-3 border-2 border-gray-200 rounded-xl font-bold text-gray-900 focus:border-brand-500 focus:ring-0 outline-none" type="number" min="0" required />
              </div>
              <div className="flex-1">
                <label className="block text-[10px] font-extrabold text-gray-500 uppercase tracking-widest mb-1.5">Max Rate (₹)</label>
                <input name="maxRate" value={newItem.maxRate} placeholder="Max Price" onChange={handleInputChange} className="w-full p-3 border-2 border-gray-200 rounded-xl font-bold text-gray-900 focus:border-brand-500 focus:ring-0 outline-none" type="number" min="0" required />
              </div>
            </div>
            <div className="relative">
              <label className="block text-[10px] font-extrabold text-gray-500 uppercase tracking-widest mb-1.5">Unit</label>
              <input name="unit" value={newItem.unit} placeholder="e.g. kg" onChange={handleInputChange} onFocus={() => setShowUnitSuggestions(true)} onBlur={() => setTimeout(() => setShowUnitSuggestions(false), 150)} className="w-full p-3 border-2 border-gray-200 rounded-xl font-bold text-gray-900 focus:border-brand-500 focus:ring-0 outline-none" required />
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="file" onChange={handleImageChange} className="w-full p-2 border-2 border-gray-200 rounded-xl text-sm bg-gray-50 font-bold text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100" />
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
            ...
            <button type="submit" disabled={!!processingId} className={`px-8 py-3 text-white rounded-xl font-extrabold flex items-center justify-center min-w-[180px] transition shadow-lg ${isEditing ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-brand-600 hover:bg-brand-700'} disabled:bg-gray-400`}>
            ...
            {items.map(item => {
              ...
              return (
                <tr key={item.id} className="bg-white hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="font-extrabold text-brand-700 bg-brand-50 px-4 py-2 rounded-xl border border-brand-100 shadow-sm">{display} / {item.unit}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
        ...
        <div id="bill-to-print" className="p-6 sm:p-10 overflow-y-auto flex-grow bg-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
            ...
            <div className="bg-brand-50 p-5 rounded-xl border border-brand-100 sm:text-right">
              ...
              <p className="font-extrabold text-xl text-brand-900">{bill.vendor?.name || 'Unknown'}</p>
              <p className="text-brand-800 font-bold text-sm mt-2 flex items-center sm:justify-end gap-2"><PhoneIcon /> {bill.vendor?.phone || 'N/A'}</p>
              <p className="text-brand-800 font-medium text-sm mt-3 border-t border-brand-200 pt-3">
                <strong>Date:</strong> {bill.createdAt ? formatDate(bill.createdAt) : 'N/A'}
              </p>
            </div>
          </div>
...
const BillingContent = ({ users, vendors, bills, openBillModal }) => {
  const sortedBills = [...bills].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
        ...
        <table className="w-full text-sm text-left text-gray-500 min-w-[800px]">
          ...
          <tbody className="divide-y divide-gray-100">
            {sortedBills.map(bill => {
              ...
              return (
                <tr key={bill.id} className="bg-white hover:bg-brand-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-bold text-gray-800">{bill.createdAt ? formatDate(bill.createdAt) : 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
        ...
        <div className="space-y-2">
          ...
          <select id="vendor-select" value={newVendorId} onChange={(e) => setNewVendorId(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-brand-500 focus:border-brand-500 block w-full p-2.5">
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button type="button" onClick={() => onConfirm(assignment.id, newVendorId)} disabled={!newVendorId || processingId === assignment.id} className="flex items-center justify-center w-32 px-4 py-2 text-sm font-medium text-white bg-brand-600 rounded-lg hover:bg-brand-700 disabled:bg-gray-400">
            ...
            {enhancedAssignments.map(a => {
              ...
              return (
                <tr key={a.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex justify-center items-center space-x-2">
                      <button onClick={() => openTransferModal(a)} className="p-2 text-brand-600 bg-brand-100 rounded-md hover:bg-brand-200" title="Transfer Order">
...
const App = () => {
  ...
  if (loading) {
    return (<div className="flex items-center justify-center min-h-screen bg-gray-100"><LoaderIcon className="w-16 h-16 animate-spin text-brand-500" /></div>);
  }
